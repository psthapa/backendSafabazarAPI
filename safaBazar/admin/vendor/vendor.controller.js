...
function addVendor(req, res, next) {
    ...
    vendor.save()
        .then((tr) => {
            (console.log("sucessfully added"), res.send(tr)) })
        .catch((err) => {
            (res.send(err)) })
}
...
function updateVendor(req, res, next) {
    ...
    const _id = req.params.id;
    Vendor.findByIdAndUpdate(_id, { $set: req.body }, { new: true }).then((vendor) => {
...
module.exports = { addVendor, getVendros, deleteVendor, updateVendor, getvendor }